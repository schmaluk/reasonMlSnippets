...
- The source code of a web app is usually splitted up into several modules of different types and those modules are referencing each other (html -> js -> css). Bundling describes the process of merging all separated modules into
a single bundle-file which can be then served as static asset by a webserver.